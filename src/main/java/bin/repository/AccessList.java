...
public class AccessList extends LinkedList<TypeMap> {
    ...
    public boolean find(String name) {
        int accessLevel = this.getAccess(name);
        name = name.substring(accessLevel);
        if (name.contains(ACCESS) || name.contains(" ") || !isLevel(accessLevel)) return false;
    ...
    public boolean find(String type, String name) {
        int accessLevel = this.getAccess(name);
        name = name.substring(accessLevel);
        if (name.contains(ACCESS) || !isLevel(accessLevel)) return false;
